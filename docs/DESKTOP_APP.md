...
選單最下方、「結束」正上方有一行灰字，用來分辨現在跑的是哪一次打包：

```
v1.0.0 · d4d9756+ · 09-16 16:14
```

| 欄位 | 來源 |
|---|---|
| `v1.0.0` | `package.json` 的 `version` |
| `d4d9756+` | 打包當下的 git commit 短碼，結尾 `+` 表示工作區有未 commit 的改動 |
| `09-16 16:14` | 打包時間 |

開發模式顯示 `v1.0.0 · 開發模式`。

資訊由 [desktop/stamp.js](../desktop/stamp.js) 在打包前寫入 `desktop/build-info.json`（已 gitignore，每次打包重產）。**因此一定要用 `npm run desktop:build`**；直接跑 `npx electron-builder` 不會戳版本，打出來的包會停留在上一次的資訊。
